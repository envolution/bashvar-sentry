...
def _parse_declare_output(output: str) -> Dict[str, Any]:
    ...
    for line in lines:
        ...
        try:
            ...
            raise ParsingError(f"Failed to parse variable '{var_name}' from line: {line}") from e
...
def source_and_get_vars(
    ...
    if not script_file.is_file():
        raise ScriptNotFoundError(f"The script '{script_path}' was not found at '{script_file}'")
    ...
    with as_file(harness_ref) as inspector_path:
        ...
        cmd = ["bash", str(inspector_path), str(script_file)]
